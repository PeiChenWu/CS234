...
class PG(object):
  ...
  def add_placeholders_op(self):
    ...
    """
    #######################################################
    #########   YOUR CODE HERE - 4-6 lines. Pei-Chen Wu  ############
    self.observation_placeholder = tf.placeholder(dtype = tf.float32, shape = [None, self.observation_dim], name = 'obs')

    if self.discrete:
      self.action_placeholder = tf.placeholder(dtype = tf.int32, shape = [None], name = 'act')
    else:
      self.action_placeholder = tf.placeholder(dtype = tf.float32, shape = [None, self.action_dim], name = 'act')
  ...
  def build_policy_network_op(self, scope = "policy_network"):
    ...
    if self.discrete:
      ...
      log_std = tf.get_variable(name = "log_std", shape = [self.action_dim], dtype=tf.float32, trainable = True)
      log_std = tf.exp(log_std)
      #log_std = tf.log(log_std + 1.)

      self.sampled_action = tf.random_normal(shape = tf.shape(action_means), mean = action_means, stddev = log_std)

      pdf = tf.contrib.distributions.MultivariateNormalDiag(loc = action_means, scale_diag = log_std)
      self.logprob = pdf.log_prob(self.action_placeholder)
  ...
  def add_loss_op(self):
    ...
    ######################################################
    #########   YOUR CODE HERE - 1-2 lines. Pei-Chen Wu  ############
  ...
  def get_returns(self, paths):
    ...
    for path in paths:
      ...
      returns = []
      for t in range(T):
        returns.append(np.sum(np.power(self.config.gamma, np.arange(T-t)) * rewards[t:]))
  ...
  def normalize_advantage(self, advantages):
    ...
    """
    #######################################################
    #########   YOUR CODE HERE - 1-5 lines. Pei-Chen Wu  ############
