...
def build_mlp(
  ...
  """
  #######################################################
  #########   YOUR CODE HERE - 7-20 lines. Pei-Chen Wu  ############
  with tf.variable_scope(scope) as _:
    x = mlp_input
    for layer in range(n_layers):
      x = tf.layers.dense(inputs = x, units = size, activation = tf.nn.relu)
    out = tf.layers.dense(inputs = x, units = output_size, activation = output_activation)
  return out
  #######################################################
  #########          END YOUR CODE.          ############