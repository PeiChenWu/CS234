...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        """
        ##############################################################
        ################YOUR CODE HERE (6-15 lines) Pei-Chen Wu ##################
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        """
        ##############################################################
        ################ YOUR CODE HERE - 2-3 lines Pei-Chen Wu ################## 
        with tf.variable_scope(scope, reuse = reuse) as _:
            x = layers.flatten(state, scope = scope)
            out = layers.fully_connected(inputs = x, num_outputs = num_actions, reuse = reuse, scope = scope, activation_fn = None)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        """
        ##############################################################
        ################### YOUR CODE HERE - 5-10 lines Pei-Chen Wu #############
        ...
        for i in range(len(q_collection)) :
            assign_list.append(tf.assign(target_q_collection[i], q_collection[i]))
    ...
    def add_loss_op(self, q, target_q):
        ...
        """
        ##############################################################
        ##################### YOUR CODE HERE - 4-5 lines Pei-Chen Wu #############
        ...
        Q = tf.reduce_sum(tf.multiply(q, indices), axis=1)
    ...
    def add_optimizer_op(self, scope):
        ...
        """
        ##############################################################
        #################### YOUR CODE HERE - 8-12 lines Pei-Chen Wu #############

        op = tf.train.AdamOptimizer(learning_rate = self.lr)
        scope_var = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope = scope)
        grads_and_vars = op.compute_gradients(self.loss, var_list = scope_var)


        if self.config.grad_clip:
            clipped_grads_and_vars = []
            for elem in grads_and_vars:
                clipped_grads_and_vars.append((tf.clip_by_norm(elem[0],self.config.clip_val), elem[1]))
        

        self.train_op = op.apply_gradients(clipped_grads_and_vars)

        self.grad_norm = tf.global_norm([elem[0] for elem in grads_and_vars])
