...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	############################
	# YOUR IMPLEMENTATION HERE - PeiChen Wu#
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	# YOUR IMPLEMENTATION HERE - PeiChen Wu#
	for state in range(nS):
		state_action_value = np.zeros(nA, dtype = float) #Q-function
		for action in range(nA):
			for probability, nextstate, reward, terminal in P[state][action]:
				state_action_value[action] += probability * (reward + gamma * value_from_policy[nextstate])
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	i = 0
	new_policy = np.zeros(nS, dtype=int)
	while i == 0 or np.linalg.norm((new_policy - policy), ord = 1) > tol:
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
	...
	while True:
		prev_value_function = np.copy(value_function)
		for state in range(nS):
			value_action = np.zeros(nA, dtype = float)
			for action in range(nA):
				for probability, nextstate, reward, terminal in P[state][action]:
					value_action[action] += probability * (reward + gamma * prev_value_function[nextstate])
			value_function[state] = np.max(value_action)
		if np.max(np.abs(prev_value_function - value_function)) < tol:
			break
